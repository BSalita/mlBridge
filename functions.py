...
def ComparePar(par, handno):
    ...
    if par.contents.parContractsString[0] == hands.parString[handno][0]:
        return False
    if par.contents.parContractsString[1] == hands.parString[handno][1]: # patch - 6-Aug-2021 - was hands.parString[handno][0]
        return False
...
def PrintDealerParBin(par): # patch - 6-Aug-2021 - added
    print("Score: {}".format(par.contents.score))
    print("Pars: {}".format(par.contents.number))

    for i in range(par.contents.number):
        ct = par.contents.contracts[i]
    
        print("Par[{}]: underTricks:{} overTricks:{} level:{} denom:{} seats:{}".format(
            i,
            ct.underTricks,ct.overTricks,ct.level,ct.denom,ct.seats))
    print()
